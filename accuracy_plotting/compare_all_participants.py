...
paths_act_i=['C:/Users/magda/PycharmProjects/Rascola_Wagner/data/1111_pe_i_example_results_action_observation.csv',
...
paths_change_i=['C:/Users/magda/PycharmProjects/Rascola_Wagner/data/1111_pe_i_example_results_new.csv',
...
for block in range(n_blocks):
    ...
    for i in range(len(paths_act)):
        data_all_trials = pd.read_csv(paths_act[i])

        data_all_env = pd.read_csv(paths_change[i], converters={'Positions': pd.eval,
                                                            'Probabilities': pd.eval})  # note, here i have to specify that the positions column will be trated as a series of lists no strings
        ...
        data_all_trials_i = pd.read_csv(paths_act[i])

        data_all_env_i = pd.read_csv(paths_change[i], converters={'Positions': pd.eval,
                                                                'Probabilities': pd.eval})  # note, here i have to specify that the positions column will be trated as a series of lists no strings
        ...
        best_count_p, mid_count_p, worst_count_p, correct_ans_p = part_block(actions, stim_1_prob_i,
                                                                                            stim_2_prob_i,
                                                                                            stim_3_prob_i, block_end,
                                                                                            block_start)
    ...
    best_count_p_all=best_count_p_all/10
    rew_best_p_all=rew_best_p_all/10
    mid_count_p_all=mid_count_p_all/10
    rew_mid_p_all=rew_mid_p_all/10
    worst_count_p_all=worst_count_p_all/10
    rew_worst_p_all=rew_worst_p_all/10
    ...
    ax=axes[block]
    #ax = plt.subplot(111)
    ax.bar(x, [best_count_p_all], width=0.2, color='mediumseagreen', alpha=0.5, align='center',
           label='correct')
    ax.bar(x, [rew_best_p_all], width=0.17, color='mediumseagreen', align='center')
    ax.bar(x2, [mid_count_p_all], width=0.2, color='yellow', alpha=0.4, align='center',
           label='2nd best')
    ax.bar(x2, [rew_mid_p_all], width=0.17, color='yellow', align='center')
    ax.bar(x3, [worst_count_p_all], width=0.2, color='red', alpha=0.5, align='center',
           label='incorrect')
    ax.bar(x3, [rew_worst_p_all], width=0.17, color='red', align='center')
    ax.set_xticks([])
...
fig.suptitle('Percepyual Pavlovian', fontsize=16)
