...
data_all_trials=pd.read_csv('/data/0_pe_p_example_results_action_observation.csv')
...
data_all_env=pd.read_csv('/data/0_pe_p_example_results_new.csv', converters={'Positions':pd.eval, 'Probabilities':pd.eval}) #note, here i have to specify that the positions column will be trated as a series of lists no strings
...
def trial_plot():
    ...
    ax[0].set_title("0, Perceptual Pavlovian")
    ...
    for outcome_ind in range(len(observations)):
        ...
        elif observations[outcome_ind]==2:
    ...
    ax[1].set_yticks([-1,1])
    ax[1].set_yticklabels(['Incorrect','Correct'])
